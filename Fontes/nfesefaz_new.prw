...
Local nValTotOpe	:= 0
Local nContImp		:= 0
Local nPosOrigExt   := 0   // Adicionado por Valdemir Jose 18/01/2013
Local nSF3Index		:= 0
Local nSF3Recno		:= 0
...
Private _cDI 	  := ""	// CCAB - importacao
Private _cContain := "" // CCAB - importacao
Private xTpFrete  := "" // CCAB - Tipo de frete
Private cCNDMT		:= SuperGetMV("MV_CNDMT", ," ")           // Paramentro que define numero CND para cada filial de MT

lCD5 := .T.     // Valdemir jose 17/04/13
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			aadd(aDest,ALLTRIM(SA1->A1_XCOMPLE))
			...
			// CCAB - Incluido por Armando em 22/07/08 para o tratamento de mensagem de local de descarte
			dbSelectArea("SA1")
			dbsetorder(1)
			dbseek(xfilial()+SF2->F2_CLIENTE+SF2->F2_LOJA)
			
			// CCAB - A condicao do tipo do cliente foi removida por Daniel em 06/08/09 conforme solicitacao do valdeci
			If /*SA1->A1_TIPO$'FL' .AND. */ !Empty(SA1->A1_XDESCAR)
				aAreaSZ1:=GetArea()
				dbSelectArea("SZ1")
				dbsetorder(1)
				dbseek(xfilial()+SA1->A1_XDESCAR)
				
				_cEndEntd := "LOCAL DE DESCARTE: "
				_cEndEntd += alltrim(SZ1->Z1_NOME) 		+ " - "
				_cEndEntd += "End: " + alltrim(SZ1->Z1_END)		+ " - "
				_cEndEntd += "Bairro: " + alltrim(SZ1->Z1_BAIRRO) 	+ " - "
				_cEndEntd += "Mun: " + alltrim(SZ1->Z1_MUN) 		+ "-"
				_cEndEntd += alltrim(SZ1->Z1_EST) 		+ " "
				_cEndEntd += "FONE: " + TRANSFORM(SZ1->Z1_TEL,"@R (99)9999-9999") + "-"
				_cEndEntd += "CEP: " + TRANSFORM(SZ1->Z1_CEP,"@R 99999-999")  + "-"
				_cEndEntd += "OBS: " + alltrim(SZ1->Z1_OBS)
				
				RestArea(aAreaSZ1)
				
			EndIf
			
			// CCAB - Incluido por Armando em 22/07/08 para tratamento de impressao de local de entrega no rodape da nota eletronica
			aAreaSA1 := GetArea()
			xPedido	 := Posicione("SD2",3,xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA,"D2_PEDIDO")
			xPrePed	 := Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_PRENUM")
			xTpFrete := Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_TPFRETE")
			xPedSFA	 := Posicione("SZ2",1,xFilial("SZ2")+xPrePed,"Z2_NUMSFA")
			
			xCliEntreg 	:= Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_CLIENT")
			xLjEntreg 	:= Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_LOJAENT")
			xCliente	:= Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_CLIENTE")
			xLoja		:= Posicione("SC5",1,xFilial("SC5")+xPedido,"C5_LOJACLI")
			
			If SA1->(dbseek(xFilial("SA1")+xCliente+xLoja)) .and. empty(xCliEntreg) .and. empty(xLjEntreg)
				...
				_cEndEnt += "FONE: "+alltrim(SA1->A1_DDD)+ " "+alltrim(SA1->A1_TEL)  + " - "+alltrim(SA1->A1_XDDDCEL)+ " "+alltrim(SA1->A1_XTELCEL)
			ElseIf SA1->(dbseek(xFilial("SA1")+xCliEntreg+xLjEntreg)) .and. !empty(xCliEntreg) .and. !empty(xLjEntreg)
				_cEndEnt := "LOCAL DE ENTREGA: "
				_cEndEnt += alltrim(SA1->A1_NOME) 		+ " - "
				_cEndEnt += alltrim(SA1->A1_ENDENT)		+ " - "
				_cEndEnt += alltrim(SA1->A1_XCOMPLI)	+ " - "
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				*/
				...
				//====================================================================================
				// Favaro - Mensagem customizada do campo "A1_XFORM01" Nova SolicitaÁ„o Faturamento ( Valdeci ) 05/12/2011                               
				//====================================================================================
				XFORM01 := Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT,"A1_XFORM01")
				
				If Alltrim(XFORM01) <> "" .AND. SF2->F2_XTPPED == '3'
					If Alltrim(cMensFis) <> ""
						cMensFis    +=  Alltrim(U_PegaForM4(SF2->F2_CLIENT,SF2->F2_LOJENT,xFilial("SF2"))) 
						cMensFis    := 	cMensFis
					Else                                                                                               
						cMensFis    :=  Alltrim(U_PegaForM4(SF2->F2_CLIENT,SF2->F2_LOJENT,xFilial("SF2")))
						cMensFis    := 	cMensFis						
...
//				aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(SA1->A1_END,"SA1")[4]))
			...
			If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA
				...
				aadd(aEntrega,ALLTRIM(SA1->A1_XCOMPLE))
		...
		EndIf
			...
			if SD2->(FieldPos("D2_DESCICM"))<>0
			   cField  +=",D2_DESCICM"						    
			EndIF
			/*			         Removido Temporariamente - Valdemir Jose 07/06/2013
			...
			*/
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD2
						...
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT, D2_PICM,      // D2_PICM ADICIONADO 15/04/2013 DEVIDO AO ERRO QUE OCORREU NA FILIAL 07
						D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_DTVALID,
						D2_ICMSRET,D2_VALPS3,D2_ORIGLAN,D2_VALCF3,D2_VALIPI %Exp:cField%,
						D2_LOCAL // CCAB - Incluido por Rodrigo em 16/02/2011
						FROM %Table:SD2% SD2
						WHERE
						SD2.D2_FILIAL  = %xFilial:SD2% AND
						SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
						SD2.D2_DOC     = %Exp:SF2->F2_DOC% AND
						SD2.D2_CLIENTE = %Exp:SF2->F2_CLIENTE% AND           // Valdemir Jose
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				Endif
					...
					Else	
		    			...
		    			EndIf
		    		EndIf
     	    		// CCAB - Agrupamento de naturezas de operacao
	    			cNatOper := fGroupTES(@aNatOper,(cAliasSD2)->D2_TES,IIf(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL,0))
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				if (SB1->B1_ORIGEM $ cOriExt) //.AND. (SB1->B1_INDUSTR='N') // Industrializado igual a "N", adicionado 08/05/2013   industr='N' removido 07/06/2013 solicitado por Valdemir e Adriana
				    nPosOrigExt := aScan(aOrigExt,{|x| ALLTRIM(X[1])=ALLTRIM(SB1->B1_DESC)})
				    if nPosOrigExt = 0
				    	aAdd(aOrigExt,{'',0} )
				    	nPosOrigExt := Len(aOrigExt)
				    Endif                  
				...
				if Substr(SD2->D2_CF,1,1)='6' .AND. (SB1->B1_INDUSTR='S')   // Verifica se est· dentro da condiÁ„o		    
				    cRetFiltra := U_FiltraFCI(SD2->D2_COD)
				    nPosFCI := aScan(aMsgFCI,{|x| ALLTRIM(X[1])=ALLTRIM(SB1->B1_DESC)})	
				    if !Empty(alltrim(TrataBuffer(@cRetFiltra)))
					    if nPosFCI = 0
					    	aAdd(aMsgFCI,{'','','',0} )
					    	nPosFCI := Len(aMsgFCI)
					    Endif                  
	...
	//				    aMsgFCI[nPosFCI][3] := Substr(aMsgFCI[nPosFCI][3],1,at('/',aMsgFCI[nPosFCI][3])-1)   // Pega Valor Custo Unitario e retira
				...
				//CCAB - Alterado por Samir para buscar a informacao do SF2 e nao do SC5
				dbSelectArea("SF2")
				dbSetOrder(1)
		  		MsSeek(xFilial("SF2")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
				If !Empty(SF2->F2_MENNOTA) .and. !(AllTrim(SF2->F2_MENNOTA) $ cMensCli)
					cMensCli += AllTrim(SF2->F2_MENNOTA)
				EndIf                                 
					
				IF Upper(Alltrim(SC5->C5_ESPECI1)) == "GRANEL"
					If !Empty(SC5->C5_XOBSERV)
						cMensCli += " " + Alltrim(SC5->C5_XOBSERV)
					EndIf
				EndIf                     
				
				IF Upper(Alltrim(SC5->C5_ESPECI1)) == "GRANEL"
					If !Empty(SC5->C5_MENNOT1)
						cMensCli += " " + Alltrim(SC5->C5_MENNOT1)
					EndIf
				EndIf				

				If !Empty(SC5->C5_MENSEXP) .and. !(AllTrim(SC5->C5_MENSEXP) $ cMensCli)
					cMensCli += " " + AllTrim(SC5->C5_MENSEXP)
				EndIf                                 
	
				// CCAB - Alterado por Armando em 22/07/08 para tratamento de impressao de campo de mensagem de nota fiscal customizado
				//Alterado por Samir para buscar a informacao do SF2 e nao do SC5
				If Alltrim(SC5->C5_MENNOT1) <> ""
				...
				// CCAB - Incluido por Armando em 22/07/08 para tratamento de mensagem padrao a ser impressa em todas as notas fiscais de acordo com solicitacao do Valdeci e Marega
				//
				// Favaro 04/04/2012
				// Tratamento somente para PA
				cMenPad := ""
				IF Alltrim(SB1->B1_TIPO) == "PA"
					//cMenPad := "PARA ATENDIMENTO DO ARTIGO 22, ITEM II DO DECR. 96044/88, CERTIFICAMOS QUE O(S)  PRODUTOS(S) ESTAO, ADEQUADAMENTE EMBALADOS(S) OU ACONDICIONADOS(S) PARA SUPORTAR OS RISCOS DO CARREGAMENTO, TRANSPORTE, DESCARREGAMENTO E TRANSBORDO. "
					cMenPad := "Para atendimento do Artigo 22, Item II do decr. 96044/88, certificamos que o(s) produto(s) estao, adequadamente embalado(s) ou acondicionado(s) para suportar os riscos do carregamento, transporte, descarregamento e transbordo."
				EndIf
				If !AllTrim(cMenPad) $ cMensCli                    
					If !Empty(cMensCli)
						cMensCli:=cMensCli+" / "
					EndIf
					cMensCli += AllTrim(cMenPad)
				EndIf
				
				// CCAB - Alterado por Armando para considerar a informacao de local de descarte em 22/07/08
				If Type("_cEndEntd")<>"U" .and. !Empty(_cEndEntd) .and. !Alltrim(_cEndEntd) $ cMensCli
					If !Empty(cMensCli)
						cMensCli+= " / "
					EndIf
					cMensCli += Alltrim(_cEndEntd)
				EndIf
					
				// CCAB - Alterado por Armando para considerar a informacao de local de origem do material em 22/07/08
				_cArmOrig := Substring(Tabela("AL",(cAliasSD2)->D2_LOCAL),RAT("/",Tabela("AL",(cAliasSD2)->D2_LOCAL))+1,Len(Tabela("AL",(cAliasSD2)->D2_LOCAL))-RAT("/",Tabela("AL",(cAliasSD2)->D2_LOCAL)))
				
				// CCAB - Alterado por Armando para considerar a informacao de local de origem do material em 22/07/08
				If !Empty(_cArmOrig) .and. !Alltrim(Formula(Alltrim(_cArmOrig))) $ cMensCli
					If !Empty(cMensCli)
						cMensCli+= " / "
					EndIf
					cMensCli := cMensCli + IIf(Empty(Formula(Alltrim(_cArmOrig))), "", Formula(Alltrim(_cArmOrig)))
				EndIf
					
				// CCAB - Alterado por Armando para considerar a informacao de local de origem do material em 22/07/08
				IF Type("xPedido") <> "U" .and. !("Ped. de Venda CCAB: " + Alltrim(xPedido)) $ cMensCli
					cMenscli += " / Ped. de Venda CCAB: " + Alltrim(xPedido)
				EndIf
				
				// CCAB - Alterado por Armando para considerar a informacao de local de origem do material em 22/07/08
				IF Type("xPrePed") <> "U"
					If !Empty(xPrePed) .and. !("PrÈ-Ped. CCAB: "+Alltrim(xPrePed) $ cMensCli)
						cMenscli += " / PrÈ-Ped. CCAB: " + Alltrim(xPrePed)
					EndIf
				EndIf
				
				// CCAB - Alterado por Armando para considerar a informacao de local de origem do material em 22/07/08
				IF Type("xPedSFA") <> "U"
					If !Empty(xPedSFA) .and. !("Ped. SFA CCAB: "+Alltrim(xPedSFA) $ cMensCli)
						cMenscli += " / Ped. SFA CCAB: " + Alltrim(xPedSFA)
					EndIf
				EndIf
				
				// CCAB - Alterado por Paulo Elias para considerar a mensagem referente a CND's por cada filial do estado de MT em 07/05/12.
				IF Type("cCNDMT") <> "U"
					IF !Empty(cCNDMT) .and. !("Num. da CND: "+Alltrim(cCNDMT) $ cMensCli)
						cMenscli += " / Num. da CND: " + Alltrim(cCNDMT)
					EndIf				
				EndIf  
					
				// CCAB - Alterado por Armando para considerar a informacao de local de entrega em 22/07/08
				If Type("_cEndEnt")<>"U" .and. !Empty(_cEndEnt) .and. !Alltrim(_cEndEnt) $ cMensCli
					If !Empty(cMensCli)
						cMensCli+= " / "
					EndIf
					cMensCli:=cMensCli+_cEndEnt                 
				EndIf
				...
				If Empty(aVeiculo)
					...
					EndIf
				EndIf
				If Alltrim(SB1->B1_GRUPO) == "700"
					If SF4->F4_XESTADO$"D"                    
						If !AllTrim(Formula("F01")) $ cMensCli
							cMensCli	+= " / " + Formula("F01")
						Endif
					Elseif SF4->F4_XESTADO$"F"						
						If !AllTrim(Formula("F02")) $ cMensCli
							cMensCli	+= " / " + Formula("F02")
						Endif
					Elseif SF4->F4_XESTADO$"A"						                    
						If !AllTrim(SF4->(Formula(F4_FORMULA))) $ cMensCli
				...
				// CCAB - Fernando 07/2011 - Tratamento para mensagens do TES
				If !Empty(POSICIONE("SF4",1,XFILIAL("SF4")+(cAliasSD2)->D2_TES,"F4_XFORMU2")) .And. !AllTrim(FORMULA(POSICIONE("SF4",1,XFILIAL("SF4")+(cAliasSD2)->D2_TES,"F4_XFORMU2"))) $ cMensCli
					If !Empty(cMensCli)
						cMensCli += " / "
					EndIf                                      
					If !AllTrim(Formula(SF4->F4_XFORMU2)) $ cMensCli
					cMensCli += AllTrim(FORMULA(POSICIONE("SF4",1,XFILIAL("SF4")+(cAliasSD2)->D2_TES,"F4_XFORMU2")))
					Endif
				EndIf
					
				// CCAB - Daniel 10/09/2010 - Tratamento para produtos especificos (caso de fertilizantes)
				_cMensGrpD := Posicione("SBM",1,xFilial("SBM")+SB1->B1_GRUPO,"BM_XMENSD")
				_cMensGrpF := Posicione("SBM",1,xFilial("SBM")+SB1->B1_GRUPO,"BM_XMENSF")

	
				//*********************** INICIO DO TRATAMENTO DA MENSAGEM SAIDA DE ARMAZEM...... - FERNANDO K. - AGILITY
				If cFilAnt == "01" .and. !EMPTY(SC5->C5_MENPAD2)
					cAux := Alltrim(Formula(SC5->C5_MENPAD2))
					If !Empty(cAux) .And. !AllTrim(cAux) $ cMensFis
						If !Empty(cMensFis)
							cMensFis += " / "
						EndIf
						cMensFis += AllTrim(cAux)
					EndIf				
				Endif    
					
				If cFilAnt == "06" .and. !EMPTY(SC5->C5_MENPAD2)
					cAux := Alltrim(Formula(SC5->C5_MENPAD2))
					If !Empty(cAux) .And. !AllTrim(cAux) $ cMensFis
						If !Empty(cMensFis)
							cMensFis += " / "
				...
				If !Empty(SF2->F2_TPFRETE)
				    ...
				    EndIf               
        		Else
					// CCAB - alterado em 04/04/11 para tratar o tipo de frete pelo pedido de venda
					If Posicione("SC5",1,xFilial("SC5")+SC5->C5_NUM,"C5_TPFRETE") == "C"
						cModFrete := "0"
					ElseIf SC5->C5_TPFRETE=="F"
					 	cModFrete := "1"
					ElseIf SC5->C5_TPFRETE=="T"
				...
				// Favaro 30/03/2012						
				cLoteForx := ""
				cLoteForx := Posicione("SB8",1,xFilial("SB8")+Padr((cAliasSD2)->D2_COD,15)+Padr((cAliasSD2)->D2_LOCAL,2)+PADR((cAliasSD2)->D2_DTVALID,8)+PADR((cAliasSD2)->D2_LOTECTL,10)+PADR((cAliasSD2)->D2_NUMLOTE,6),"B8_LOTEFOR")
				//======================================================================
				// Novo Ajuste para pegar o Codigo de Barras do Fornecedor quando Houver	
				// Favaro 04/04/2012						
				cCodBarx  := ""
	            cCodBarx  := Posicione("SA5",2,xFilial("SA5")+PADR((cAliasSD2)->D2_COD,15),"A5_CODBAR")
				...
				//-----------------------------------------------------------------------------------------
				If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6" .And. cCSTrib $ "00-10-20-70-90") .And. ((cAliasSD2)->D2_PICM == 4)
					cMsgFci	:= "Resolucao do Senado Federal n∫ 13/12"
					lFCI	:= .T.                      
					/*        Removido Temporariamente Valdemir JosÈ 07/06/2013
				...
				aadd(aProd,	{Len(aProd)+1,;
					...
					SB1->B1_UM,;
					(cAliasSD2)->D2_QUANT,;
					IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL,0),;   //IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+nDesconto+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
					...
					(cAliasSD2)->D2_VALFRE,;
					(cAliasSD2)->D2_SEGURO,;
					0,;// CCAB - Substituido por daniel em 27/04/2011 (nDesconto+nDescIcm),;
					IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN,0),;// CCAB - Substituido por daniel em 27/04/2011 RetPrvUnit(cAliasSD2,nDesconto),;
					IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF
					IIF(ALLTRIM(cLoteForx) <> "","Lote For. "+cLoteForx,IIF(!Empty((cAliasSD2)->D2_LOTECTL),"Lote: "+(cAliasSD2)->D2_LOTECTL,"Sem Lote")),;
					...
					nDescZF,;
					(cAliasSD2)->D2_TES,;					//(cAliasSD2)->D2_TES}) - TROCADO POR OS CAMPOS ABAIXOS 04/06/2013
					(cAliasSD2)->D2_TOTAL,;
					SB1->B1_CODISS,;
					IIF(SB1->(FieldPos("B1_IMPNCM"))<>0,SB1->B1_IMPNCM,0)})					
					...
					Do Case
						...
						Case AllTrim(CD2->CD2_IMP) == "ISS" 
							...
							If Empty(aISS)
								aISS := {0,0,0,0,0}
							EndIf
							aISS[01] += (cAliasSD2)->D2_TOTAL // CCAB - Substituido por daniel em 27/04/2011 (cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCON
							aISS[02] += CD2->CD2_BC
		    ...
		    if Len(aOrigExt) > 0
               ...
               For nOriExt := 1 To Len(aOrigExt)
                   ...
                   //cMsgOriExt  += if(nPosFCI > 0,' Nro. FCI: '+aMsgFCI[nPosFCI][2]+'Nro. CI: '+aMsgFCI[nPosFCI][3]+'% VLR IMPORT.'+alltrim(Transform(aOrigExt[nOriExt][2],"@E 999,999,999.99")) ,'')  // Valdemir 24/04 /    Removido Valdemir 08/05/13 ( removido por solicitaÁ„o das regras contabeis)
               Next
		    ...
		    if !lFCI 
               if Len(aMsgFCI) > 0
	               cMsgOriExt  := ' - RESOL.SEN.FED.N∫ 13/12, ' 
	               For nOriExt := 1 To Len(aMsgFCI)
	                   IF nOriExt > 1
	                   	cMsgOriExt  += ' / '
	                   ENDIF
	                   cMsgOriExt  += alltrim(aMsgFCI[nOriExt][1])
	                   cMsgOriExt  += ' FCI N∫ '+alltrim(aMsgFCI[nOriExt][2])+' '
			...
			Endif
			if !Empty(SA1->A1_XDANFE2)
				cMensCli  += ' '+alltrim(SA1->A1_XDANFE2)
			Endif                                   
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			EndIf
			// CCAB - Daniel - 04/06/2009
			If Alltrim(SF1->F1_XOBSERV) <> ""
				cMensCli += Alltrim(SF1->F1_XOBSERV)
			EndIf		
			
			// CCAB - Daniel - 04/06/2009
			// aadd(aEspVol,{SF1->F1_ESPECI1, SF1->F1_VOLUME1, SF1->F1_PLIQUI , SF1->F1_PBRUTO})	
			// CCAB - Mensagens de importacao
			_cDI := IIf(SF1->F1_EST="EX" .AND. !Empty(SF1->F1_XDI)    ,"* * * D.I. " + AllTrim(SF1->F1_XDI) + " de " + Dtoc(SF1->F1_XDATADI),"")
			_cDI += IIf(SF1->F1_EST="EX" .AND. !Empty(SF1->F1_XLOCDES),"* * * Mercadoria saindo de " + SF1->F1_XLOCDES                       ,"")			
			If SF1->F1_TIPO $ "DB" 
			    dbSelectArea("SA1")
				dbSetOrder(1)
...
//				aadd(aDest,MyGetEnd(SA1->A1_END,"SA1")[4])
			...
			#IFDEF TOP
				...
				BeginSql Alias cAliasSD1			
						...
						D1_LOCAL,D1_OBS,D1_XNUMCON // CCAB - Incluido por Rodrigo em 16/02/2011
						FROM %Table:SD1% SD1
						WHERE
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				// CCAB - Daniel - 04/06/2009
				If !Empty(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_FORMULA")) .And. !AllTrim(Formula(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_FORMULA"))) $ cMensFis
					If !Empty(cMensFis)
						cMensFis += " / "
					EndIf
					cMensFis += AllTrim(Formula(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_FORMULA")))
				EndIf

				If !Empty(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_XFORMU2")) .And. !AllTrim(Formula(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_XFORMU2"))) $ cMensFis
					If !Empty(cMensFis)
						cMensFis += " / "
					EndIf
					cMensFis += AllTrim(Formula(Posicione("SF4",1,xFilial("SF4")+(cAliasSD1)->D1_TES,"F4_XFORMU2")))
				EndIf				
				cNatOper := fGroupTES(@aNatOper,(cAliasSD1)->D1_TES,IIf(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_TOTAL,0))	    		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					SB1->B1_UM,;
					(cAliasSD1)->D1_QUANT,;
					IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_TOTAL,0),;// CCAB - Substituido por daniel em 27/04/2011 IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_II),0),;
					IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,SB5->B5_UMDIPI),;
					IIF(Empty(SB5->B5_CONVDIPI),(cAliasSD1)->D1_QUANT,SB5->B5_CONVDIPI*(cAliasSD1)->D1_QUANT),;
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
					(cAliasSD1)->D1_VALDESC,;
				   	IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_VUNIT,0),;     // IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_VUNIT,((cAliasSD1)->D1_VUNIT)-((cAliasSD1)->D1_II/(cAliasSD1)->D1_QUANT)),0),;
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
				...
				ElseIf (!lEasy) //.AND. (!lCD5) removido devido a mudanÁa do novo fonte Valdemir 04/06/2013
					DbSelectArea("CD5")
					DbSetOrder(4)
					...
					EndIf
				Else
					// CCAB - 11/03/11 - NF  de importacao
					If SF1->F1_FORMUL=="S" .AND. SF1->F1_EST=="EX"
						DbSelectArea("SD1")
						DbSetOrder(1)
						
						DbSeek(xFilial("SD1")+SF1->(F1_DOC+F1_SERIE+F1_FORNECE+F1_LOJA)+(cAliasSD1)->(D1_COD+D1_ITEM))
						aadd(aDI,U_Enfe001())
						aAdi := aDI
						
						If !Empty((cAliasSD1)->D1_XNUMCON) .and. !(AllTrim((cAliasSD1)->D1_XNUMCON) $ _cContain)
							_cContain += AllTrim((cAliasSD1)->D1_XNUMCON) + ","
						EndIf
					Else					
						aadd(aAdi,{})
						aadd(aDi,{})
				...
				If MsSeek(xFilial("SFT")+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD) .And. ;
					...
					IF Empty(aPis[Len(aPis)]) .And. !empty(SFT->FT_CSTPIS) .And. (SFT->FT_CSTPIS$"09,06,04,99" .Or. SFT->FT_CSTPIS$"70")
						aTail(aPisAlqZ):= {SF4->F4_CSTPIS}						
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. !empty(SFT->FT_CSTCOF) .And. (SFT->FT_CSTCOF$"09,06,04,99" .Or. SFT->FT_CSTCOF$"70")
						aTail(aCofAlqZ):= {SF4->F4_CSTCOF}					
					EndIf

				Else

					IF Empty(aPis[Len(aPis)]) .And. !empty(SF4->F4_CSTPIS) .And. (SF4->F4_CSTPIS$"09,06,04,99" .Or. SF4->F4_CSTPIS$"70")
						aTail(aPisAlqZ):= {SF4->F4_CSTPIS}	
					EndIf
					IF Empty(aCOFINS[Len(aCOFINS)]) .And. !empty(SF4->F4_CSTCOF) .And. (SF4->F4_CSTCOF$"09,06,04,99" .Or. SF4->F4_CSTCOF$"70") 
						aTail(aCofAlqZ):= {SF4->F4_CSTCOF}	
					EndIf
...
// CCAB - Mensagens de importacao
If !Empty(_cDI)
	cMensCli += Alltrim(_cDI)
Endif
If  !Empty(_cContain)
	cMensCli += "Container(s) - " + Substr(_cContain,1,Len(_cContain)-1)
Endif
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],aMed[nX],aArma[nX],aveicProd[nX],aDI[nX],aAdi[nX],aExp[nX],aPisAlqZ[nX],aCofAlqZ[nX],aAnfI[nX],cTipo,cVerAmb, aComb[Nx],@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX],aDest,cIpiCst,@nTotTrib)
	Next nX
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst, nTotTrib)   // REMOVIDO UM PARAMETRO POR J¡ EXISTIR - VALDEMIR JOSE 07/06/2013
...
If cVerAmb== "2.00" 
	...
	If cTipo == "1"
		...
		//nTributo := ConvType((aprod[28]  * nAliquota)/100,15,2) - Removido por solicitaÁ„o do Paulo CCAB 10/06/2013 - Valdemir
		// Adicionado por Valdemir Jose 10/06/2013
		nTributo := SF2->(F2_VALICM+F2_VALIPI+F2_VALISS+F2_VALIMP1F2_VALIMP2+F2_VALIMP3+F2_VALIMP4+F2_VALIMP5+F2_VALIMP6+F2_VALINSS+F2_VALCSLL+F2_VALCOFI+F2_VALPIS+F2_VALIRRF)
		cString += NfeTag('<vTotTrib>' ,nTributo)
		nTotTrib := nTributo   // += nTributo
	EndIf	
Endif                             
...
cLoteProd := aProd[19]   // aProd[20] LOTE:
...
if !empty(aProd[15]) .And. !empty(cMotDesICMS) 
	...
	dbSelectArea("SB5")  // adicionado por Valdemir Jose
	dbSetOrder(1)
	If MsSeek(xFilial("SB5")+aProd[2])
		cMensDeson := ALLTRIM(SB5->B5_XDESCR1)+" "+ALLTRIM(SB5->B5_XDESCR2)+" "+ALLTRIM(SB5->B5_XDESCR3)+" "+ALLTRIM(SB5->B5_XDESCR4)+" "+ALLTRIM(SB5->B5_XDESCR5)+" "+ALLTRIM(SB5->B5_XDESCR6)+" "+ALLTRIM(SB5->B5_XDESCR7)   // Valdemir 15/04/2013
	endif
endif  

MsSeek(xFilial("SF4")+aProd[27]) 
if SF4->(FieldPos("F4_MSGLT"))>0
	if SF4->F4_MSGLT $ "1" .And. !empty(aProd[19])    //aProd[20]
		if !empty(aProd[25])
	...
	IF Empty(aProd[19])
 	...
 	cString += '<infadprod>'+cLoteProd+" "+cMensDeson+'</infadprod>'     // ConvType(aProd[25],500)
endif
...
If nTotTrib > 0    // !Empty(cPercTrib) .And.
...
±±≥FunáÖo	 ≥fGroupTES ≥ Autor ≥ Felipe AurÈlio de Melo ≥ Data ≥ 27.02.09 ≥±
...
±±≥Parametros≥ cD2TES   = CÛdigo da TES do item                            ≥±
...
//aNatOper[x][1] == Codigo TES
//aNatOper[x][2] == Descr. TES
//aNatOper[x][3] == Vlr. Total
...
For x:=1 To Len(aNatOper)
	//Trata para n„o mostrar mais que trÍs descriÁıes
	If x > 3
		x := Len(aNatOper)

	Else
		//Trata para n„o duplicar descriÁıes
...
±±∫Programa  ≥NFESEFAZ_NEW∫Autor  ≥ Favaro           ∫ Data ≥  12/02/11   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Pega Formula Inteligente por Filial                        ∫±±
±±∫          ≥                                                            ∫±±
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
*/                                                
User Function PegaForM4(cCliM4,cLojaM4,cFilM4) // Parametos Cliente Loja Filial do Faturamento
cTextM4AT := ""
cTextM4PX := ""
aForM4 := {}
cTextoM4 := alltrim(Posicione("SA1",1,xFilial("SA1")+cCliM4+cLojaM4,"A1_XFORM01"))

cTextoM4 := StrTran(cTextoM4,"//",";")
// Separa as Formulas por Filial, de tras para frente ( facilitador )   
While .T.
	nXSep := RAT(";",ctextoM4)            
	cTForm := Substr(ctextoM4,nXSep+1,5)
	aadd(aForM4,cTForm)
	cTextoM4 := Substr(cTextom4,1,nXSep-1)
	If alltrim(cTextoM4) == ""
		Exit
	EndIf
End      

For xM4 := 1 To Len(aForM4)
	If Substr(aForM4[xM4],1,2) <> cFilM4
		Loop
	EndIf
	cTextM4PX := posicione("SM4",1,aForM4[xM4],"M4_FORMULA")
	cTextM4AT := Alltrim(cTextM4AT) + Alltrim(cTextM4PX)
Next xM4
Return(cTextM4AT)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±…ÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕª±±
±±∫Programa  ≥NFESEFAZ_NEW∫Autor  ≥Valdemir Jose     ∫ Data ≥  18/04/13   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Carrega Informacao com base na TES e SD2                   ∫±±
±±∫          ≥                                                            ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ AP                                                         ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
...
ﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂﬂ
*/
Static Function MsgM4(cTes, cFilM4)
	Local cRET       := ""
	Local _cMensF    := ""
	Local _cGrupo    := ""
	Local _cMensF    := ""
	...
	if dbSeek(xFilial('SF4')+cTes)
		...
		Else
			_cMensTes:= ""
			If SF4->F4_XESTADO$"D"
				IF !EMPTY(SF4->F4_FORMULA) .AND. (ALLTRIM(SA1->A1_EST)='EX')      // Valdemir Jose
					_cMensTes	+= Formula(SF4->F4_FORMULA)
				ELSE
					If (!AllTrim(Formula("F01")) $ _cMensTes) .and. (!Empty(Formula("F01")))   // Empty - Acrescentado por Valdemir JosÈ 07/02/13
			...
			Elseif SF4->F4_XESTADO$"F"
				IF !EMPTY(SF4->F4_FORMULA) .AND. (ALLTRIM(SA1->A1_EST)='EX')      // Valdemir Jose
					_cMensTes	+= Formula(SF4->F4_FORMULA)
				ELSE
					If (!AllTrim(Formula("F02")) $ _cMensTes)  .and. (!Empty(Formula("F02")))   // Empty - Acrescentado por Valdemir JosÈ 07/02/13
			...
			Else
				IF !EMPTY(SF4->F4_FORMULA) .AND. (ALLTRIM(SA1->A1_EST)='EX')      // Valdemir Jose
					_cMensTes	+= Formula(SF4->F4_FORMULA)
				ELSE
					If (!AllTrim(SF4->(Formula(F4_FORMULA))) $ _cMensTes).and. (!Empty(SF4->F4_FORMULA))   // Empty - Acrescentado por Valdemir JosÈ 07/02/13
						_cMensTes	+= Formula(SF4->F4_FORMULA)
					Endif
				ENDIF
			Endif
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±…ÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕª±±
±±∫Programa  ≥M460MARK  ∫Autor  ≥Valdemir Jose       ∫ Data ≥  24/04/13   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Filtra informaÁoes do numero da FCI, conforme regra passada∫±±
±±∫          ≥ por Paulo                                                  ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Uso       ≥ AP                                                         ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±…ÕÕÕÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÀÕÕÕÕÕÕ—ÕÕÕÕÕÕÕÕÕÕÕÕÕª±±
±±∫Programa  ≥TrataBuffer  ∫Autor  ≥Valdemir JosÈ    ∫ Data ≥  09/05/13   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Desc.     ≥ Rotina que far· a quebra das informaÁıes-"|"               ∫±±
±±∫          ≥                                                            ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Retorno   ≥ Conteudo                                                   ∫±±
±±»ÕÕÕÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕº±±
